...
export class PromptSystem {
  ...
  private getBehaviorRules(): string {
    ...
    3. Режим ответов:
      * ПО УМОЛЧАНИЮ (режим "ai_decides"): отвечай только когда тебя упоминают по имени "Петал".
      * НЕ отвечай на обычную болтовню между пользователями, если они не обращаются к тебе.
      * Если видишь сообщения других пользователей без упоминания тебя - просто наблюдай, не вмешивайся.
      * Отвечай только когда:
        - Тебя упомянули напрямую ("Петал", "петал")
        - Обратились к тебе с вопросом или просьбой
    `;
  }
